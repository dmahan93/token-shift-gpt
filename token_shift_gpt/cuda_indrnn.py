...
"""
Repurposed for token shift gpt, original source here: https://github.com/Sunnydreamrain/IndRNN_pytorch/blob/master/cuda_IndRNN_onlyrecurrent.py
- Updated to the latest pytorch autograd function (non static forward&backward -> static forward and backward)
- Optional static weight
- Removed relu activation because it's not needed for this application
"""
...
class IndRNN_Compute_GPU(Function):
    ...
    @staticmethod
    def forward(self, x, weight_hh, h0, funcs):
    ...
    def backward(self, grad_h):
        ...
        return grad_x, grad_weight_hh, grad_h0, None
        ...
        for name, weight in self.named_parameters():
            if "weight_hh" in name:
                if self.recurrent_init is None:
                    nn.init.uniform_(self.weight_hh, 0.0, 1.0)
                else:
                    self.recurrent_init(weight)

    def forward(self, input, weight=0.99, h0=None):
        ...
        if weight is not None:
            return IndRNN_Compute.apply(input, torch.ones_like(self.weight_hh)*weight, h0, IndRNN_Compute.get_functions())
        else:
            return IndRNN_Compute.apply(input, self.weight_hh, h0, IndRNN_Compute.get_functions())